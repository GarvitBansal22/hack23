...
@Component
public class CountUpdateStream {
    ...
    private final Logger logger = Logger.getLogger(CountUpdateStream.class);
    ...
    private JsonObject extractPayload(JsonObject value){
        if (value == null)
            return new JsonObject();
        if (this.isSchemaEnabled && value.has("payload"))
            return value.get("payload").getAsJsonObject();
        return value;
    }
